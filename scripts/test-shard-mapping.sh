...
[
 ...
 {"kv_key":"lb:company:active:{employees:99999999999999999999}", "want":"lb.company.sx", "want_shard":"unparseable",
  "note":"id >= 2^63 overflows the .int64() cast -> sx isolation (same as a missing id); must NOT error the mapping (a mapping error leaves kv_prefix unset -> kv.cdc.lb.company.null.<op> -> XACKed, no consumer match -> silent loss)"},
 {"op":"rename",
 ...
 {"op":"rename", "kv_key":"lb:company:active:{employees:3}",
  "old_key":"lb:company:standby:{employees:5}", "new_key":"lb:company:active:{employees:5}",
  "want":"lb.company.sx", "want_shard":"cross_shard_rename",
  "note":"kv_key (s3) disagrees with old/new (s1): routing key vs touched keys split -> quarantine"},
 {"op":"rename",
  "old_key":"lb:company:standby:{employees:5}", "new_key":"lb:company:active:{employees:99999999999999999999}",
  "want":"lb.company.sx", "want_shard":"unparseable",
  "note":"rename new_key id >= 2^63 overflows: x_shard arithmetic .catch(false) falls through to sx+unparseable (kv_key absent -> new_key is the shard key). NOT cross_shard_rename — overflow is an unplaceable id, not an INV-S7 violation, so cross_shard_rename stays 0"},
