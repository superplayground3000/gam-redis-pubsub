...
| Topology manifest (§7) | L1 (`enabled:` toggle) + L3 (mismatch fail-closed — the `verify-multi-env.sh` gate now requires the `topology DRIFT` hook signal, not a bare nonzero helm rc, to credit the block; unreadable fail-mode discriminated by a `_verified_<env>` KV marker written after a successful compare — marker present ⇒ redeploy fail-open, absent ⇒ first-install fail-closed — so a first install whose durables were precreated for the handoff is NOT misread as a redeploy) |
