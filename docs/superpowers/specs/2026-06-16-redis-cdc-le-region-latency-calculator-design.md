...
unchanged), append a **best-effort** latency emit:

- `writer_ts` = `body.ts` parsed from the stashed `meta("body")` JSON. If `body`
  has no `ts` (e.g. verifier-injected events), **skip** the sample (no XADD).
- `sink_ts` = current unix-millis at apply time.
- `XADD cdc:latency MAXLEN ~ 50000 * op <op> kv_key <key> writer_ts <wts> sink_ts <sts>`
  against region Redis (reuses `rrcs.redis.region.url`).
- Wrapped in `try`/`catch`: on XADD failure, `log` a warning and **clear the
  error** in `catch` so the real apply still ACKs. Telemetry never causes a
  nack / redelivery / double-apply.
...
- Stream name `cdc:latency` and `MAXLEN` are templated from values.
...
best-effort). No central/NATS access anywhere.
...
file is always current). All latencies in **milliseconds**.
...
- `chart/templates/latency-calculator.yaml` — a Deployment (1 replica), gated
  ...
  components). Mounts an `emptyDir` at `/reports` (PVC optional via a values
  flag). Standard `rrcs.podLabels` / `rrcs.scheduling` / image helpers.
- `chart/values.yaml` — new `latencyCalculator` block:
  ...
  Region Redis URL reuses `rrcs.redis.region.url`.
...
- nearest-rank percentile incl. empty, single-sample, and small-N edge cases;
