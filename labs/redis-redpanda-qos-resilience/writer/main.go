...
var (
	writesTotal = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "rrqr_writer_writes_total",
		Help: "Successful (SET+XADD) writes to central Redis, per pattern.",
	}, []string{"pattern"})

	writeErrorsTotal = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "rrqr_writer_write_errors_total",
		Help: "Failed writes to central Redis, per pattern.",
	}, []string{"pattern"})

	profileInfo = promauto.NewGaugeVec(prometheus.GaugeOpts{
		Name: "rrqr_writer_profile_info",
...
func main() {
	...
	go func() {
		...
		log.Printf("health+metrics listening on %s", healthAddr)
	...
	log.Printf("writer started: addr=%s stream=%s interval=%dms profile=%s keys=%d",
		addr, streamKey, intervalMs, profile, len(keys))
	...
	for {
		...
		case <-tick.C:
			...
			if _, err := pipe.Exec(ctx); err != nil {
				writeErrorsTotal.WithLabelValues(k.Pattern).Inc()
				log.Printf("write error key=%s err=%v", k.Key, err)
				continue
			}
			writesTotal.WithLabelValues(k.Pattern).Inc()
