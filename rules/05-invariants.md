...
| `chart/files/connect/cdc-reverse.yaml` | `cdc_sync_latency_seconds{op}` timing records only after a successful apply (`!errored()` + `sync_has_ts` guard); its value stays clamped ≥ 0 and `.int64().string()`-formatted — a negative or float value makes the metric processor error AFTER the apply and nack an already-applied message. Negative deltas increment `cdc_sync_skew_negative` instead. **Opt-in `prefix` label (`connect.syncLatencyPrefix.enabled`, default off, 2026-07-23):** enabled render emits `{op,prefix}`; the label derives ONLY from `meta("nats_subject")` (bounded set: routing families + `others` + `none`; a sharded publisher's trailing `s<K>` token is stripped), never from `kv_key` (INV-S8 spirit — unbounded cardinality). The derivation lives in the same post-apply mapping, so it must stay meta-only and throw-proof (`.or("")` + length-guarded slice/index) for exactly the nack reason above. Toggle off keeps the default render byte-identical (the L1 merge-base guard enforces this). **The opt-in per-message sync-latency log (`connect.syncLatencyLog.enabled`, default off, 2026-07-29) sits in the SAME post-apply branch and keeps the same defensive posture:** every `fields_mapping` read is meta-only with `.or()`/`.catch()` fallbacks. (Verified on the pinned build 2026-07-29: a `log` fields_mapping throw is SWALLOWED — internal ERROR line, fields dropped, message continues un-errored — unlike the `metric` processor's hard nack. The guards exist so the log line stays usable, not because ack safety depends on them) |
...
| `chart/files/grafana/cdc-dashboard.json` | Panels exist for: apply throughput, unprocessable activity, unprocessable-by-reason, processor errors, forward-leg publish failures, Connect latency p50/p95/p99, end-to-end latency percentiles, sync latency p50/p95/p99 by prefix (grouping label `prefix` — emitted only under `connect.syncLatencyPrefix.enabled`; unlabeled samples collapse into one blank-prefix series), sync clock-skew negatives, writer throughput/errors, elector leadership, and **panel 18 "DLQ: routed vs confirmed parked"** — its `routed` series is `sum by (reason, shard)` so it covers the sharded pipeline's `cdc_dlq_forwarded{shard,reason}` with a per-shard breakdown, and `confirmed parked`/`publish failures` read `output_{sent,error}{label=dlq_out}` across `job=~".*connect-sink.*"` (sharded per-group jobs included). **If you add a metric, add/extend a panel in the same change.** |
