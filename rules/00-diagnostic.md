...
| Makefile / CI | None. Entry points are scripts in `scripts/` and `go test ./...` |
| Tests | `go test ./...` (<10s); kind e2e `scripts/verify-cdc.sh` (~5 min); chaos `scripts/verify-failover.sh` (~12 min) |
...
- **Fix:** The verification ladder in `rules/05-invariants.md` §Verification ladder maps change types to required test levels (L0 unit → L3 kind e2e → L4 failover chaos). Fast container/docker-compose verification (L2) is a known gap — see `rules/05-invariants.md` §Known gaps.
...
- No CI means "run after every change" is enforced only by these rules. Adding CI is listed as follow-up work in `rules/05-invariants.md` §Known gaps.
